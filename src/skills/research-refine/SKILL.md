...
For wiki grounding, read the user's wiki contract at `<WIKI_PATH>` before reading wiki pages. Look for `AGENTS.md`, `CLAUDE.md`, `README.md`, or `ingest_prompt.md`. Use the contract's schema and retrieval expectations; do not hard-code the wiki layout. If no contract exists, use read-only markdown fallback and report that the wiki contract was missing.
...
experiment-plan "idea:<slug>"                    -> detailed experiment roadmap
...
4. `experiment-plan` turns the final proposal into a detailed claim-driven experiment roadmap
...
- `src/skills/experiment-plan/SKILL.md` — downstream claim-driven planning.
